...
- 오늘의 루틴 기록 (DAY1~4, 세트별 무게/횟수 입력, 메모, 숫자 검증)
  ...
  - 60/90/120초 휴식 타이머
- 마이페이지
  - 상단에 🔥연속 기록일 · 🗓️총 기록일 · 🏋️총 볼륨(무게×횟수 합) 요약 표시
  - 🏅 개인 최고기록(PR) + 🗓️ 날짜별 기록 히스토리 (수정/삭제 가능)
  - ⚙️ 계정 설정: 닉네임 변경, 비밀번호 변경, 회원 탈퇴(본인이 직접, 비밀번호 확인 후)
- 랭킹
  - 👑 전체 종목 1위: 23개 운동 각각의 현재 챔피언(닉네임/무게/횟수)을 한 번에 확인
  - 📋 종목별 TOP 20: 운동 하나를 골라 순위표(최고 무게, 동률 시 최고 횟수)로 확인
  - 🏋️ 총 볼륨 랭킹: 모든 운동의 무게×횟수를 합산한 전체 유저 순위 (꾸준함 동기부여용)
...
- 비밀번호 찾기(분실 시 복구) 기능은 아직 없음 — 이메일/전화번호를 아예 안 받는 구조라, 필요하면 가입 시 이메일 필드를 추가하고 비밀번호 재설정 토큰 발급 로직을 붙여야 함
- "운동 추가 요청" 문의가 들어와도 실제 운동 추가는 `utils/exercises_data.json`을 직접 수정하고 재배포해야 함 — 관리자가 웹에서 바로 운동을 추가/수정하게 하려면 이 데이터를 DB로 옮기는 작업이 필요함
