...
1. 상단의 **🧩 루틴 → 루틴 만들기**에서 이름을 입력하고 운동을 선택합니다.
2. 표의 순서·세트·목표 횟수를 정한 뒤 루틴을 저장합니다.
3. **저장 루틴**에서 `시작`을 누르면 순차 운동 세션이 열립니다.
4. 지난 기록을 확인하거나 `지난 기록 그대로 채우기`를 누르고 오늘 세트를 기록합니다.
5. 모든 운동을 마친 뒤 `오늘 운동 종료`를 누르면 세션이 완료됩니다.
6. **주간 계획**에서 요일별 루틴을 지정하면 오늘 화면에 해당 루틴이 먼저 추천됩니다.
7. 체형 기록과 확장 통계는 **📖 기록 → 체형 / 훈련 통계**에서 확인합니다.
...
루틴 데이터 정규화, 세션 생성, 공식/개인 운동 병합, 비활성 운동 처리, 체형 입력 검증,
점진적 과부하, 예상 1RM, 주간 비교 및 부위별 볼륨 계산을 자동 테스트합니다.
