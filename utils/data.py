...
# ================= 목표 시간 기반 추천 프리셋 =================
# "부위를 몇 개, 종목을 몇 개 해야 할지 모르겠다"는 헬린이를 위해
# 부위/종목수를 직접 고르지 않아도 되는 미리 정의된 조합.
# minutes는 그 조합으로 운동했을 때 대략 걸리는 시간(세트+휴식 포함, 안내용).
PART_PRESETS = [
    {
        "key": "full",
        "label": "전신",
...
PART_PRESETS_BY_KEY = {p["key"]: p for p in PART_PRESETS}
...
UPDATE_LOG = [
    {
        "date": "2026-08-27",
        "items": [
            "🧩 '나만의 루틴'과 '오늘 뭐 할지 정하기'를 하나의 진입점('오늘 뭐 할지 정하기')으로 통합",
            "🎯 첫 화면에 전신(~60분)·상체만(~40분)·하체만(~30분)·부위 직접 선택 버튼을 바로 두고, 목표만 고르면 부위·종목 수까지 자동 계산해서 추천",
            "🚀 추천받은 목록으로 바로 운동을 시작하거나, 오늘 화면에만 담아둘 수 있음",
            "📁 저장한 루틴 시작 / ✅ 직접 고르기 / 🎲 웜업 중 1개만 추천은 '다른 방법으로 정하기'에서 계속 이용 가능",
            "부위를 여러 개 골라 한 번에 추천할 때 같은 운동이 두 부위에 걸쳐 중복으로 뽑히던 문제 수정",
        ],
    },
    {
        "date": "2026-08-24",
        "items": [
            "✅ 360px 모바일 전 화면 점검 완료: 체형·루틴·운동 세션·통계·관리자·계정·뱃지의 입력칸과 버튼 잘림 수정",
            "📱 오늘 운동 부위·기록·랭킹·루틴·관리자 화면을 한 번에 하나만 여는 모바일 메뉴로 개선해 스크롤과 로딩 부담 감소",
            "운동 세션의 세트 입력은 무게·횟수를 한 줄에 유지하고, 나머지 일반 입력칸은 모바일에서 안전하게 세로 배치",
            "체형 기록 날짜를 바꾸면 해당 날짜의 기존 체중·골격근량·체지방률·메모를 불러와 안전하게 수정 가능",
            "점진적 과부하는 계획한 모든 세트를 기록하고 목표를 달성한 경우에만 증량을 추천하도록 정확도 개선",
            "0회 운동 기록과 100% 초과 체지방률을 차단하고, 챔피언·랭킹 조회의 중복 DB 요청을 줄여 성능 개선",
            "📱 모바일 입력 화면 개선: 체중·골격근량·체지방률 등 여러 입력칸이 화면 밖으로 잘리지 않고 세로로 표시되도록 수정",
            "🎲 초보자 자동 루틴 추천 추가: 운동할 부위와 종목 수만 고르면 공식 운동을 골고루 랜덤 추천",
            "추천·선택된 운동마다 필요한 기구와 동작 방법을 바로 확인할 수 있도록 루틴 만들기 화면 개선",
            "모바일에서 넓게 잘리던 루틴 편집 표를 운동별 카드와 세트·목표 횟수 입력 방식으로 교체",
