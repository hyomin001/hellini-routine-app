...
def render_tier_card(workout_days: int):
    """총 기록일(누적 운동일수) 기준 헬린이 등급 카드. 0일은 흰색, 1~10일/11~30일/... 1년 이상까지
    구간별로 색이 달라지고, 다음 등급까지 며칠 남았는지도 같이 보여준다."""
    tier = get_tier(workout_days)
    if tier["max"] is None:
    ...
    st.markdown(
        ...
        f"🗓️ 총 {workout_days}일 기록 · {range_txt} 구간</div>"
        "</div>",
        unsafe_allow_html=True,
    )
    if tier["next"]:
        need = max(0, tier["next"]["min"] - workout_days)
